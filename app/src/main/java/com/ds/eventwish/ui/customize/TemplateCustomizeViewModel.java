...
public class TemplateCustomizeViewModel extends ViewModel {
    ...
    public void loadTemplate(String templateId) {
        ...
        // Use the getTemplates() method which returns LiveData and filter locally
        templateRepository.getTemplates().observeForever(templates -> {
            if (templates != null && !templates.isEmpty()) {
                for (Template candidateTemplate : templates) {
                    if (candidateTemplate.getId() != null && 
                        candidateTemplate.getId().equals(templateId)) {
                        this.template.setValue(candidateTemplate);
                        updatePreview();
                        return;
                    }
                }
                // Template not found
                error.setValue("Template not found with ID: " + templateId);
            } else {
                error.setValue("No templates available");
            }
        });
    }
