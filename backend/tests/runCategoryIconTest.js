/**
 * Modified test script for CategoryIcon handling in Templates
 * 
 * This test verifies:
...
async function createTestCategoryIcon() {
  ...
  await icon.save();
  console.log(`Created test icon with ID: ${icon._id} and custom id: ${icon.id}`);
  
  // Verify that both _id and id are accessible
  const iconJSON = icon.toJSON();
  console.log(`Icon JSON _id: ${iconJSON._id}, id: ${iconJSON.id}`);
...
async function createTestTemplate(categoryIcon) {
  ...
  await template.save();
  console.log(`Created test template with ID: ${template._id} referencing icon: ${template.categoryIcon}`);
  return template;
}

// Test direct CategoryIcon model behavior
async function testCategoryIconModel() {
  console.log('Testing CategoryIcon model behavior');
...
async function testTemplateWithCategoryIcon(icon) {
  console.log('Testing Template with CategoryIcon reference');
